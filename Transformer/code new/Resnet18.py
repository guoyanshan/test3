...
class Resnet18(nn.Module):
    ...
    def forward(self, ms, ms_up, pan_down_up, pan, phase='train'):
        ...
        # Compute attention
        attn = torch.matmul(Q1, K1.transpose(-2, -1))

        # Normalization(Softmax)
        attn = F.softmax(attn, dim=-1)

        # Attention output
        spectral_output = torch.matmul(attn, Vm1).view(b, c, h, -1)
        spacial_output = torch.matmul(attn, Vp1).view(b, c, h, -1)
        # print(spectral_output.shape)
        # print(spacial_output.shape)

        out = []
        if phase == 'train':
            # loss
            spectral_loss = self.mutual_information_loss(spectral_output, f_pan_5)
            spacial_loss = self.mutual_information_loss(spacial_output, Vm)
            out.append(spectral_loss+spacial_loss)

        # f_fusion = torch.cat([  spectral_output, spacial_output], 1)
        # print(f_fusion.shape)
        # s = f_fusion.view(-1, 1024)
        # print(s.shape)
        # rel = self.outlayer(s)
        # out.append(rel)
        # return out
        ...
        f_fusion = torch.cat([  spectral_output, spacial_output], 1)
        f_fusion = F.adaptive_avg_pool2d(f_fusion, [1, 1])
